...
export default function ForwarderMasterPage() {
  ...
  const dashboardStats = useMemo(() => {
    const active = rawForwarders.filter(f => f.entityStatus === 'ACTIVE');
    const totalAgencies = active.length;
    const avgRating = totalAgencies > 0 
      ? Number((active.reduce((acc, curr) => acc + curr.rating, 0) / totalAgencies).toFixed(2))
      : 0;
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    const payload: Partial<Forwarder> = {
      name,
      email,
      phone,
      address,
  ...
  return (
    <MasterPage 
      title="Forwarder Matrix" 
      subtitle="Logistics Forwarding Agencies & Port Coverage Control"
      loading={loading}
    >
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              ...
              <div className="relative w-full max-w-lg flex items-center bg-white/5 border border-white/10 rounded-2xl overflow-hidden focus-within:border-blue-500/50 transition-all font-mono">
                ...
                >
                  ...
                  <option value="name" className="bg-[#0b0b0b]">Agency</option>
              ...
              {showFilterDrawer && (
                ...
                >
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-6 text-[10px] font-mono">
                    ...
                    {/* Ports Filter */}
                    <div className="space-y-2">
                      <p className="text-white/70 uppercase tracking-wider">Ports Covered</p>
                      <div className="flex flex-wrap gap-1.5">
                        {filterOptions.ports.map(port => (
                          <button
                            ...
                            onClick={() => setFilters(prev => ({
                              ...prev,
                              ports: prev.ports.includes(port) ? prev.ports.filter(x => x !== port) : [...prev.ports, port]
                            }))}
                            className={cn(
                              "px-2.5 py-1 rounded bg-[#101010] border text-[9px]",
                              filters.ports.includes(port) ? "border-blue-500 text-blue-400 bg-blue-500/5" : "border-white/5 text-white/70"
                            )}
                          >
                            {port}
                          </button>
                  ...
                  <div className="flex justify-end gap-3 mt-6 border-t border-white/5 pt-4">
                    <button 
                      onClick={() => setFilters({ countries: [], ports: [], statuses: [] })}
                  ...
                  <tr>
                    ...
                    <th className="py-5 px-6">
                      <button 
                        ...
                        className="flex items-center gap-2 hover:text-white transition-colors bg-transparent border-none cursor-pointer text-white/70 text-xs font-mono uppercase"
                      >
                        Rating {sortBy === 'rating' && (sortOrder === 'asc' ? <ChevronUp size={10} /> : <ChevronDown size={10} />)}
                      </button>
                    </th>
                  ...
                  {forwarders.map((f) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        <div>
                          <p className="font-sans font-bold text-sm text-white/90 truncate max-w-[200px]">{f.name}</p>
                          <p className="text-[9px] text-white/25 truncate max-w-[200px]">{f.email}</p>
                        </div>
                      </td>
                      <td className="py-4 px-6 text-white/70">{f.country}</td>
                      <td className="py-4 px-6">
                      ...
                      <td className="py-4 px-6">
                        <div className="flex flex-wrap gap-1">
                          {(f.preferredPorts || []).map(port => (
                            <span key={port} className="px-1.5 py-0.5 rounded bg-white/5 text-white/90 text-[8px] border border-white/5">{port}</span>
                          ))}
                        </div>
                      </td>
                      <td className="py-4 px-6 text-right">
                        <span className={cn(
                          ...
                          f.entityStatus === 'ACTIVE' && "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20",
                          f.entityStatus === 'ARCHIVED' && "bg-amber-500/10 text-amber-400 border border-amber-500/20",
                          f.entityStatus === 'INACTIVE' && "bg-white/5 text-white/70 border border-white/10"
                        )}>{f.entityStatus}</span>
                      </td>
                    </tr>
                  ))}
            ...
            >
              ...
              <div className="glass p-8 rounded-4xl border border-white/5 relative overflow-hidden">
                  ...
                  <div className="flex-1 min-w-0 pr-12">
                    <div className="flex items-center gap-2 mb-3">
                      <div className="flex items-center gap-1 px-2.5 py-0.5 rounded bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[8px] font-mono font-bold uppercase tracking-widest">
                        <Star size={8} fill="currentColor" /> {activeForwarder.rating.toFixed(1)} Rating
                      </div>
                      <span className={cn(
                    ...
                    <h2 className="text-2xl font-display font-medium tracking-tight text-white mb-2 truncate">{activeForwarder.name}</h2>
                    <p className="text-[10px] font-mono text-white/80 tracking-wider mb-2">TAX: {activeForwarder.taxId || 'N/A'} • REGION: {activeForwarder.country}</p>
                    <a 
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 border-t border-white/5 pt-6">
                  {[
                    { label: 'Rating score', value: `★ ${activeForwarder.rating.toFixed(1)}`, icon: Star, color: 'text-amber-400' },
              ...
              <div className="glass p-8 rounded-4xl border border-white/5">
                ...
                {detailTab === 'contacts' && (
                  <div className="space-y-4">
                    {activeForwarder.contacts.map((c, idx) => (
                ...
                {detailTab === 'ports' && (
                  <div className="space-y-3">
                    <p className="text-[9px] font-mono text-white/70 uppercase tracking-wider mb-2">Preferred Ports of Operations</p>
                    <div className="flex flex-wrap gap-2">
                      {(activeForwarder.preferredPorts || []).map(portId => {
                        const portObj = ports.find(p => p.id === portId);
                        return (
                          <div key={portId} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/2 border border-white/5 font-mono text-xs text-white/80">
                ...
                {detailTab === 'history' && (
                  <div className="space-y-3 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
                    <p className="text-[9px] font-mono text-white/70 uppercase tracking-wider mb-3 font-bold">Managed Shipments</p>
                    <div className="space-y-2">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="p-4 rounded-2xl bg-white/2 border border-white/5 space-y-3">
                      <div className="flex justify-between items-center">
                        <p className="text-[8px] font-mono text-white/70 uppercase tracking-wider">Log Communication</p>
                        <select
                          value={commType}
          ...
          <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/70 backdrop-blur-md">
            ...
            >
              ...
              <form onSubmit={handleFormSubmit} className="space-y-6">
                ...
                {formTab === 'general' && (
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-[9px] font-mono text-white/80 uppercase block">Logistics Agency Name</label>
                        <input 
                          type="text" 
                          required
                          value={name}
                      ...
                      <div className="space-y-2">
                        <label className="text-[9px] font-mono text-white/80 uppercase block">Booking Email</label>
                        <input 
                          type="email" 
                          required
                          value={email}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div className="space-y-2">
                        <label className="text-[9px] font-mono text-white/80 uppercase block">Agency Tax ID / Registration</label>
                        <input 
                          type="text" 
                          value={taxId}
                          onChange={(e) => setTaxId(e.target.value)}
                          className="w-full bg-[#0b0b0b] border border-white/10 rounded-xl py-3 px-4 text-xs font-mono text-white focus:outline-none"
                          placeholder="e.g. TAX-FWD-123"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-[9px] font-mono text-white/80 uppercase block">Agency Website URL</label>
                        <input 
                          type="text" 
                          value={website}
                    ...
                    <div className="space-y-2">
                      <label className="text-[9px] font-mono text-white/80 uppercase block">Office Address Details</label>
                      <textarea 
                        value={address}
                ...
                {formTab === 'ports' && (
                  <div className="space-y-4">
                    <span className="text-[9px] font-mono text-white/80 uppercase block mb-2">Preferred Ports Covered</span>
                ...
                {formTab === 'contacts' && (
                  <div className="space-y-6">
                    <div className="flex justify-between items-center">
                      <span className="text-[9px] font-mono text-white/80 uppercase tracking-wider">Associated Agents / Port Managers</span>
                      <button 
                        type="button"
                      ...
                      {formContacts.map((contact, idx) => (
                        <div key={idx} className="p-5 rounded-2xl bg-[#080808] border border-white/10 relative space-y-4 font-mono">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <label htmlFor={`contact-primary-${idx}`} className="text-[9px] font-mono text-white/70 cursor-pointer">Mark as Primary Agency Contact</label>
                          </div>
                        </div>
                      ))}
                    </div>
