...
export default function ShipmentsPage() {
  ...
  return (
    <>
      <PageHeaderUpdater title="Global Logistics Hub" subtitle="Real-time Shipment Matrix & Freight Pipeline" />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        {[
          ...
          <div key={i} className="glass p-5 rounded-3xl border border-border flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest mb-0.5 truncate">{item.label}</p>
              <p className="font-sans font-bold text-base text-foreground">{item.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-6">
            ...
            {showFilters && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6 text-[10px] font-mono">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {filterOptions.statuses.map(s => (
                        <button key={s} onClick={() => setFilters(f => ({ ...f, statuses: f.statuses.includes(s) ? f.statuses.filter(x => x !== s) : [...f.statuses, s] }))}
                          className={cn('px-2.5 py-1 rounded bg-card border text-[9px]', filters.statuses.includes(s) ? 'border-blue-500 text-blue-400' : 'border-border text-muted-foreground')}>
                          {s.replace('_', ' ')}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="space-y-2">
                    <p className="text-muted-foreground uppercase tracking-wider">Shipping Line</p>
                    <div className="flex flex-wrap gap-1.5">
                      {filterOptions.shippingLines.map(l => (
                        <button key={l} onClick={() => setFilters(f => ({ ...f, shippingLines: f.shippingLines.includes(l) ? f.shippingLines.filter(x => x !== l) : [...f.shippingLines, l] }))}
                          className={cn('px-2.5 py-1 rounded bg-card border text-[9px]', filters.shippingLines.includes(l) ? 'border-blue-500 text-blue-400' : 'border-border text-muted-foreground')}>
                          {l}
                        </button>
                      ))}
                    </div>
        ...
        <div className="space-y-4">
          {shipments.map(shp => (
            <div key={shp.id} className="glass rounded-3xl border border-border hover:border-border transition-all group overflow-hidden">
              <div className="p-6">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
                  ...
                  <div className="lg:col-span-3 flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <Link href={`/shipments/${shp.id}`}>
                        <p className="font-sans font-bold text-foreground/90 group-hover:text-blue-400 transition-colors text-sm">{shp.shipmentNo}</p>
                      </Link>
                      <p className="text-[9px] font-mono text-muted-foreground truncate">{shp.containerType} · {shp.shippingLineId}</p>
                    </div>
                  </div>
                  ...
                  <div className="lg:col-span-5 flex items-center gap-4">
                    <div className="text-right min-w-[80px]">
                      <p className="text-[9px] font-mono text-muted-foreground uppercase">Origin</p>
                      <p className="font-bold text-muted-foreground text-sm font-mono">{shp.originPortId}</p>
                      <p className="text-[9px] text-muted-foreground font-mono">{formatDate(shp.etd)}</p>
                    </div>
                    <div className="flex-1 flex flex-col items-center gap-1">
                      <div className="w-full relative h-1 bg-muted rounded-full overflow-hidden">
                        <motion.div
                          initial={{ width: 0 }}
                      ...
                      <div className="flex items-center gap-1">
                        <motion.div animate={{ x: [-2, 2, -2] }} transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
                          className={cn("text-[10px]", ['TRANSIT', 'ON_VESSEL'].includes(shp.status) ? 'text-blue-400' : 'text-muted-foreground')}>
                          <Ship size={12} />
                        </motion.div>
                    ...
                    <div className="min-w-[80px]">
                      <p className="text-[9px] font-mono text-muted-foreground uppercase">Destination</p>
                      <p className="font-bold text-muted-foreground text-sm font-mono">{shp.destinationPortId}</p>
                      <p className="text-[9px] text-muted-foreground font-mono">{formatDate(shp.eta)}</p>
                    </div>
                  </div>

                  {/* Vessel & Freight */}
                  <div className="lg:col-span-2 text-xs font-mono">
                    <p className="text-[9px] text-muted-foreground uppercase mb-0.5">Vessel</p>
                    <p className="text-foreground/90 truncate text-[10px]">{shp.vesselName || '—'}</p>
                    <p className="text-[9px] text-muted-foreground mt-1">{shp.voyageNo || ''}</p>
                  </div>

                  {/* Actions */}
                  <div className="lg:col-span-2 flex justify-end items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Link href={`/shipments/${shp.id}`}>
                      <button className="p-2.5 rounded-xl hover:bg-accent text-muted-foreground hover:text-foreground bg-transparent border-none cursor-pointer" title="View detail"><Eye size={14} /></button>
                    </Link>
                    {!['COMPLETED', 'CANCELLED'].includes(shp.status) && (
                      <button onClick={() => advance(shp.id)}
                        className="p-2.5 rounded-xl hover:bg-blue-500/10 text-muted-foreground hover:text-blue-400 bg-transparent border-none cursor-pointer" title="Advance to next stage">
                        <FastForward size={14} />
                      </button>
                    )}
                    {!['COMPLETED', 'CANCELLED'].includes(shp.status) && (
                      <button onClick={() => cancelShipment(shp.id)}
                        className="p-2.5 rounded-xl hover:bg-rose-500/10 text-muted-foreground hover:text-rose-400 bg-transparent border-none cursor-pointer" title="Cancel">
                        <XCircle size={14} />
                      </button>
                    )}
                    <button onClick={() => softDelete(shp.id)}
                      className="p-2.5 rounded-xl hover:bg-accent text-muted-foreground hover:text-rose-400 bg-transparent border-none cursor-pointer" title="Delete">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
                ...
                {/* Progress bar bottom */}
                <div className="mt-4 w-full h-px bg-muted">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${STATUS_PROGRESS[shp.status] || 0}%` }}
          ...
          {shipments.length === 0 && (
            <div className="glass rounded-3xl border border-border py-16 text-center text-muted-foreground font-mono text-xs uppercase tracking-widest">
