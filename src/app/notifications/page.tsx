  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto space-y-8 pb-20">
        <div className="glass p-6 md:p-8 rounded-4xl border border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex gap-2 p-1 bg-white/5 rounded-xl border border-white/5 w-full md:w-auto overflow-x-auto">
            <button 
              ...
              className={cn(
                ...
                filter === 'ALL' ? "bg-white text-black shadow-lg" : "bg-transparent text-white/70 hover:text-white"
              )}
            >
              All Directives
            </button>
            <button 
              ...
              className={cn(
                ...
                filter === 'UNREAD' ? "bg-white text-black shadow-lg" : "bg-transparent text-white/70 hover:text-white"
              )}
            >
              Unread
              {unreadCount > 0 && (
                <span className={cn(
                  ...
                  filter === 'UNREAD' ? "bg-black text-white" : "bg-white/10 text-white"
                )}>{unreadCount}</span>
              )}
            </button>
          </div>
          ...
          <button 
            ...
            className="w-full md:w-auto flex justify-center items-center gap-2 px-6 py-3 bg-white/5 border border-white/10 rounded-xl text-[10px] font-mono uppercase tracking-widest hover:bg-white/10 transition-colors cursor-pointer disabled:opacity-50"
          >
        ...
        <div className="space-y-4">
              ...
              <motion.div 
                ...
                className={cn(
                  ...
                  notif.isRead 
                    ...
                    : "bg-white/5 border-white/10 shadow-lg shadow-black/20 hover:bg-white/[0.07]"
                )}
              >
                {!notif.isRead && (
                ...
                <div className="flex-1 min-w-0 w-full">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="text-[10px] font-mono text-white/80 uppercase tracking-tighter whitespace-nowrap shrink-0 mt-1">
                      {new Date(notif.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </div>
                  
                  <p className="text-xs md:text-sm text-white/80 leading-relaxed mb-4 md:mb-6">{notif.message}</p>
          ...
          {filtered.length === 0 && (
            <div className="py-24 text-center glass rounded-4xl border border-white/5">
              ...
              <p className="text-sm font-mono text-white/80 uppercase tracking-widest">Stream is clear.</p>
            </div>
          )}
        </div>
      </div>
