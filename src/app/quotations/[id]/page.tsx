...
export default function QuotationDetailPage() {
  ...
  const handleSaveNote = async () => {
    ...
    const updatedTimeline = [timelineEvent, ...(quotation.timeline || [])];
  ...
  return (
    <MasterPage 
      ...
      subtitle={`Commercial proposal version v${quotation.version || 1}.0`}
      loading={loading}
    >
                  ...
                  <tbody className="divide-y divide-white/5">
                    {quotation.items.map((item, idx) => (
                      <tr key={idx} className="text-white/80">
                        <td className="py-4">
                          <p className="font-sans font-bold text-white/95">{getProductName(item.productId)}</p>
                        </td>
